...
r = requests.get("http://127.0.0.1/downloads/files/local/wifi.gco")
if r.status_code != 200:
...
print "Now cleaning up, deleting http://127.0.0.1/api/files/local/wifi.gco"
d = requests.delete('http://127.0.0.1/api/files/local/wifi.gco', headers={'X-Api-Key':API_KEY})
if not d.status_code == 204:
    print "Expected status code 204 after deleting the wifi.gco file"
    sys.exit(1)
